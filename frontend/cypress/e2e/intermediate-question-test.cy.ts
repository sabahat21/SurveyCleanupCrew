...
describe('Beginner Question Quiz Page', () => {
 before(() => {
  ...
  //Sign into admin dashboard
    ...
    cy.get('select').first().select('History');
    cy.get('select').last().select('Text');
    cy.get('textarea').type('Test Intermediate 2');
...
beforeEach(() => {
  ...
  it('Test Intermediate Level Text', () => {
...
it('Test Survey Progress text', () => {
    cy.get('.text-xl')
    .should("exist")
    .should("have.text","Survey Progress");
});
...
it('Test Save & Continue Error Message', () => {
  ...
  cy.contains("Please answer the question or click Skip.").should('be.visible');
});
...
it('Test Skip all question displays survey result', () => {
  ...
  cy.get('.text-lg')
  .should('have.text', 'Survey Complete!');
});

it('Test Skip all question displays survey result', () => {
  skipAllQuestions() 

  cy.get('.relative > .px-4')
  .should('have.text', 'SURVEY Level');
});
...
it('Test submission answer Survey Success text', () => {
    ...
    cy.get('.bg-white > .mb-6')
    .should("exist")
    .should("have.text","Survey submitted successfully! Thank you for your participation.")
});
